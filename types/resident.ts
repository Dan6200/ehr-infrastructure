...
export const createResidentConverter = (encryptionKey: string): FirestoreDataConverter<Resident> => ({
  toFirestore(resident: Resident): DocumentData {
    ...
    if (encryptedResident.emergencyContacts) {
      encryptedResident.emergencyContacts = encryptedResident.emergencyContacts.map((contact) => {
  ...
  fromFirestore(snapshot: QueryDocumentSnapshot<DocumentData>): Resident {
    ...
    if (data.emergencyContacts && Array.isArray(data.emergencyContacts)) {
      decryptedData.emergencyContacts = data.emergencyContacts.map((contact: any) => {
